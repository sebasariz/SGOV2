 ...
 */
public class RutaRepartoMongoController {
...
//        System.out.println("objInc: " + objInc);
    ...
    public int getNextID() {
        ...
        if (!dBCursor.hasNext()) {
            DBCollection collection = database.getCollection("rutaReparto");
    ...
    public RutaReparto createRutaServicio(RutaReparto rutaReparto) {
        rutaReparto.setIdRutaServicio(getNextID());
        String jsonString = gson.toJson(rutaReparto);
        DBObject rutaRepartoMONGO = (DBObject) JSON.parse(jsonString);
        MongoClient mongoClient = SessionControl.getMongoClient();
        DB database = mongoClient.getDB(propertiesAcces.mongoDB);
        DBCollection collection = database.getCollection("rutaReparto");
        WriteResult result = collection.insert(rutaRepartoMONGO);
        DBCollection collectionPosicion = database.getCollection("rutaReparto");
        return rutaReparto;
    }

    public RutaReparto editRutaServicio(RutaReparto rutaReparto) {
        String jsonString = gson.toJson(rutaReparto);
        DBObject rutaRepartoMONGO = (DBObject) JSON.parse(jsonString);
        MongoClient mongoClient = SessionControl.getMongoClient();
        DB database = mongoClient.getDB(propertiesAcces.mongoDB);
        DBCollection collection = database.getCollection("rutaReparto");
        collection.update(new BasicDBObject("idRutaServicio", rutaReparto.getIdRutaServicio()), rutaRepartoMONGO);
        return rutaReparto;
    }

    public RutaReparto removeRutaServicio(RutaReparto rutaReparto) {
        MongoClient mongoClient = SessionControl.getMongoClient();
        DB database = mongoClient.getDB(propertiesAcces.mongoDB);
        DBCollection collection = database.getCollection("rutaReparto");
        collection.remove(new BasicDBObject("idRutaServicio", rutaReparto.getIdRutaServicio()));
        return rutaReparto;
    }
