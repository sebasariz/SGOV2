...
function init(grid) {
    ...
    var tableHisotrial = $('#datatable-historial').DataTable({
        "order": [[0, "desc"]],
        "language": {
            "url": "//cdn.datatables.net/plug-ins/9dcbecd42ad/i18n/Spanish.json"
        },
...
function LoadHistorialJson(id) {
    $("#modal-esperar").modal("show");
    $.ajax({
        url: "LoadHistorialClienteJson.sga",
        data: {idEmpresa: id}
    }).done(function (json) {
...
function addItem() {
    ...
    $.get("pages_in/item.jsp", function (itemHtml) {
        itemHtml = itemHtml.replace("$itemtext", itemText);
...
//            console.log(key + ": " + data)
        ...
        $.get("pages_in/item_entrega.jsp", function (itemHtml) {
            for (var i = 0; i < array.length; i++) {
                ...
                inicial = inicial.replace("$total", valorVenta);
